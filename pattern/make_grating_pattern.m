...
x = 1:pattern.x_num;
Pats(:,mod(x,2*bar_width)>=bar_width,1,bb)  = bar_brightness(bb);
% for gg = 0:2:floor(160/bar_width)
% Pats(:, floor(gg*bar_width)+1:floor((gg+1)*bar_width), 1, bb) = bar_brightness(bb);
% end
...
% Save
str = fullfile(pat_dir, ['Pattern_20x4_grating_width_' num2str(bar_width) '_brightness_' num2str(min(bar_brightness)) '-' num2str(max(bar_brightness)) '.mat']);
save(str, 'pattern');
