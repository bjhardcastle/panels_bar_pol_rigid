...
bar_brightness = 0:2^pattern.gs_val-1;
pat_dir = cd;
...
pattern.gs_val = 3; 	% This pattern will use 4 intensity levels
...
Pats(:, round(pattern.x_num/3)-floor(0.5*bar_width)+1:round(pattern.x_num/3)+floor(0.5*bar_width), 1, bb) = bar_brightness(bb);

Pats(:, round(2*pattern.x_num/3)-floor(0.5*bar_width)+1:round(2*pattern.x_num/3)+floor(0.5*bar_width), 1, bb) = bar_brightness(bb);
...
% Save
str = fullfile(pat_dir, ['Pattern_20x4_three_bright_bar_width_' num2str(bar_width) '_brightness_' num2str(min(bar_brightness)) '-' num2str(max(bar_brightness)) '.mat']);
save(str, 'pattern');
